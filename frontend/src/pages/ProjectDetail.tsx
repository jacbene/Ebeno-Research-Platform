  ...
  const fetchProjectData = async () => {
    ...
    try {
      ...
      if (transData.success) {
        const all = transData.data.transcriptions || [];
        setTranscriptions(all.filter((t: any) => t.type === 'audio'));
        setTextDocuments(all.filter((t: any) => t.type === 'text'));
      }

      const memoRes = await fetch(`http://localhost:5001/api/memos?projectId=${id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (memoRes.ok) {
      ...
      const filesRes = await fetch(`http://localhost:5001/api/projects/${id}/files`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (filesRes.ok) {
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: theme.spacing.sm }}>
                  ...
                  <span style={{ fontSize: '12px', color: colors.gray[500] }}>
                    {new Date(result.created_at || result.uploaded_at).toLocaleDateString()}
                  </span>
                </div>
      ...
      <div>
        ...
        {activeTab === 'documents' && (
          <Card title="📁 Documents du projet">
            ...
            {/* Panneau d'actions pour le document sélectionné */}
            {selectedDocument && (
              <DocumentActions
