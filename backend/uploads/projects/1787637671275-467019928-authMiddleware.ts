...
import { prisma } from '../lib/prisma'; // Import ajouté

// Déclaration globale cohérente avec auth.ts
declare global {
  namespace Express {
    interface Request {
      user?: {
...
export const requireOwnership = (resourceOwnerId: string) => {
  return (req: Request, res: Response, next: NextFunction) => {
    if (req.user?.id !== resourceOwnerId) {
      return res.status(403).json({
        success: false,
        message: 'Accès refusé. Vous ne possédez pas cette ressource.'
      });
    }
    next();
  };
    ...
    try {
      ...
      if (!userId || !projectId) {
        return res.status(400).json({
          success: false,
          message: 'ID utilisateur ou projet manquant'
        });
      }
